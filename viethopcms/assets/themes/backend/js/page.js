$(function () {
        ...
        .bootstrapValidator({
            ...
            fields: {
                ...
                post_article_content: {
                    ...
                    validators: {
                        callback: {
                            message: 'Page content is not less than 0 characters',
                ...
                post_menu: {
                    ...
                    validators: {
                        notEmpty: {
                            message: 'Select the menu for the page.'
                        },
                        callback: {
                            message: 'Select the menu for the page',
                            callback: function(value, validator, $field) {
    ...
    $(document).on('click', '.update_article', function () {
        ...
        if (validator.isValid()) {
            ...
            {
                swal("","Opp! something wrong","error");
                return false;
            }
            let datapost = {};
            let article_status = $(this).attr('status');
            ...
            $.ajax({
                ...
                setTimeout(function () {
                    ...
                    $('#submit_publish').prop('disabled', false);
                    $('#submit_draft').prop('disabled', false);
                    swal("","Update page success!","success");
                }, 10);
    ...
    $(document).on('click', '#ajax_add_menu', function () {
        if ($('.ajax_menu').val() == '') {
            $('.menu_alert').html('Please input menu first.');
            setTimeout(function () {
                $('.menu_alert').html('Select the parent menu if needed.');
            }, 3000);
        }
        else {
            $.ajax({
