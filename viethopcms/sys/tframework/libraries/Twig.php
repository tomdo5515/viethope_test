...
if(!defined('TWIGGY_ROOT')) define('TWIGGY_ROOT', dirname(dirname(dirname(__FILE__))));

//If you don't like composer but love sparks. POSER!
//if ( !class_exists('ComposerAutoloaderInit') )
require_once(TWIGGY_ROOT . '/third_party/Twig/Autoloader.php');
...
class Twig
{
	...
	*/
	...
	public function register_function($name, $callable = NULL)
	{
		//$this->_twig->extensionInitialized = false;
		$this->_twig->addFunction($name, new Twig_SimpleFunction($name, $callable));
		//$this->_twig->extensionInitialized = true;
		return $this;
	}
	...
	public function register_filter($name, $callable)
	{
		$this->_twig->addFilter($name, new Twig_SimpleFilter($name, $callable));
	...
	*/       	
	...
	public function theme($theme)
	{
		...
		//set TimeZone
		// $this->_twig->getExtension('core')->setTimezone($this->_config['timezone']);
		...
		if(count($this->_config['register_functions']) > 0)
		{
			...
			{
				$this->register_function($function, $function);
			}
		}
		...
		if(count($this->_config['register_filters']) > 0)
		{
			...
			{
				$this->register_filter($filter, $filter);
			}
		}

		//Initialized after line, can't add function, filter...
		$this->_twig->setLexer(new Twig_Lexer($this->_twig, $this->_config['delimiters']));

		// Initialize defaults
		$this->layout($this->_config['default_layout'])
	...
	/**
	 * Display the compiled HTML content use CI Output
	 *
	 * @access	public
	 * @param 	string	(optional) template file
	 * @return	void
	...
	public function display($template = '')
	{
		...
		try
		{
			$this->CI->output->append_output($this->_load()->render($this->_data));
		}
		catch(Twig_Error_Loader $e)
		{
			show_error($e->getRawMessage());
	...
	/**
	 * Display the compiled HTML content direct
	 *
	 * @access	public
	 * @param 	string	(optional) template file
	 * @return	void
	...
	public function outscreen($template = '')
	{
		if(!empty($template)) $this->template($template);
		...
		try
		{
			...
			$this->CI->output->_display($this->_load()->render($this->_data));
		}
		catch(Twig_Error_Loader $e)
		{
			show_error($e->getRawMessage());
	...
	private function _set_template_locations($theme)
	{
		...
		if(method_exists($this->CI->router, 'fetch_module'))
		{
        	...
        	if ( $current_module !== $this->_module )
	        {
				...
				{
		            if ( is_dir($loc . $current_module. '/views' ) )
							$this->_template_locations[] = $loc . $current_module. '/views' ; 
				}
	        }
	        /**/
			...
			if(!empty($this->_module))
			{
				...
				foreach($module_locations as $loc => $offset)
				{
					...
					/* [johnny edit 24-02-2016] Only add the template location id the same exists, otherwise this load template 
					default of module */
					if ( is_dir($loc . $this->_module. '/views' ) )
						$this->_template_locations[] = $loc . $this->_module. '/views' ;
				}
			}
		}
