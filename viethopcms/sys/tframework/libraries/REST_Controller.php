...
abstract class REST_Controller extends MX_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($this->auth_override === FALSE && ($this->config->item('allow_auth_and_keys') === TRUE && $this->_allow === TRUE))
        {
            $rest_auth = strtolower($this->config->item('rest_auth'));
            switch ($rest_auth)
            {
                case 'aes':
                    $this->_prepare_aes_auth();
                    break;
                case 'basic':
    ...
    /**
     * Prepares for AES authentication
     * tick,appid,ramdom(5)
     * @access protected
     * @return void
     */
    protected function _prepare_aes_auth()
    {
        ...
        //FUqFlgaxN/whEdogTjKCS0FWZAaJgy0m4rUuAvZRmlA=
        $ciphertext = $this->input->get_request_header($this->config->item('rest_asekey_name'), TRUE);
        if($ciphertext != NULL)
        {
            $tick = $this->input->get('tick');
            if($tick != NULL)
            {

                $this->load->library('myencrypt', $this->config->item('rest_encryption_key'));

                // $plain_text = time().',test,1234';//'1470294844,test,1234';
                // $ciphertext = $this->myencrypt->encrypt($plain_text);
                // echo $ciphertext.PHP_EOL; die;

                $plain_text = $this->myencrypt->decrypt($ciphertext);

                $array_variable = explode(',', $plain_text);

                if (sizeof($array_variable) > 2) {
                    
                    if (!in_array($array_variable[1], $this->config->item('rest_appid')) || $array_variable[0]!=$tick) {
                        $this->_force_login();
                    }

                    $check = time() - $tick;
                    
                    if($check > 3000) //50 minutes
                        $this->_force_login();
                }else {
                    $this->_force_login();
                }
