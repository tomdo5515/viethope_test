 ...
 * Site key and secret key can be obtained from https://www.google.com/recaptcha/admin/, while language code can be 
 */
...
class ReCaptcha {
    ...
    private $_secret, $_sitekey, $_lang, $_theme, $_size;
    ...
    function __construct() {
        ...
        if ($this->ci->config->item('theme', 'recaptcha') == NULL || $this->ci->config->item('theme', 'recaptcha') == "") {
            $this->_theme = 'light';
        } else {
            $this->_theme = $this->ci->config->item('theme', 'recaptcha');
        }

        if ($this->ci->config->item('size', 'recaptcha') == NULL || $this->ci->config->item('size', 'recaptcha') == "") {
            $this->_size = 'normal';
        } else {
            $this->_size = $this->ci->config->item('size', 'recaptcha');
        }
    }
    ...
    public function render() {
        $return = '<div class="g-recaptcha" data-sitekey="' . $this->_sitekey . '" data-theme="' . $this->_theme . '" data-size="' . $this->_size . '"></div>
