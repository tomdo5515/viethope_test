...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		// joh add extra functions
		$data->extra_functions		= $this->extra_functions;
	...
	{
		$this->set_css($this->default_assets_path.'/plugins/bootstrap-select/css/bootstrap-select.min.css');
		$this->set_js_lib($this->default_assets_path.'/plugins/bootstrap-select/js/bootstrap-select.min.js');
	}
	...
	{
		$this->set_js_lib($this->default_texteditor_path.'/ckfinder/ckfinder.js');
		$this->set_js_lib($this->default_texteditor_path.'/ckfinder/instance_popup.js');
	}
	...
	protected function get_true_false_input($field_info,$value)
	{
		...
		$input = "<div class=\"row\">";
		$input .= "<div class=\"animated-radio-button col-md-1 col-xs-6\">";
		$input .= "<label for='field-{$field_info->name}-true'>";
		$input .= "<input id='field-{$field_info->name}-true' type=\"radio\" name=\"{$field_info->name}\" value=\"1\" $checked><span class=\"label-text\">$true_string</span>";
		$input .= "</label>";
		$input .= "</div>";
		...
		$input .= "<div class=\"animated-radio-button col-md-1 col-xs-6\">";
		$input .= "<label for='field-{$field_info->name}-false'>";
		$input .= "<input id='field-{$field_info->name}-false' type=\"radio\" name=\"{$field_info->name}\" value=\"0\" $checked><span class=\"label-text\">$false_string</span>";
		$input .= "</label>";
		$input .= "</div>";
		$input .= "</div>";
	...
	{
		$this->set_js_lib($this->default_assets_path.'/plugins/datepicker/moment.min.js');
		$this->set_css($this->default_assets_path.'/plugins/datepicker/bootstrap-datetimepicker.min.css');
		$this->set_js_lib($this->default_assets_path.'/plugins/datepicker/bootstrap-datetimepicker.min.js');

		$this->set_js_config($this->default_assets_path.'/plugins/datepicker/bootstrap-datetimepicker.config.js');
		...
		$input ='<div class="form-line row">';
		$input .='<div class="col-xs-12">';
		$input .= "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$datetime' maxlength='19' class='form-control datetimepicker'/>";
		$input .='</div>';
		$input .='</div>';
		return $input;
	}
	...
	protected function get_dropdown_input($field_info,$value)
	{
		...
		// added by joh
	        ...
	        if ($state === 'add')
	            $value = $default_value;
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}' class='selectpicker' data-live-search='true' data-placeholder='".$select_title."'>";
	...
	protected function get_set_input($field_info,$value)
	{
		...
		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}[]' size='8' class='selectpicker' multiple data-live-search='true' data-placeholder='$select_title' style='width:510px;' >";
	...
	protected function get_multiselect_input($field_info,$value)
	{
		...
		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}[]' size='8' class='selectpicker' multiple data-placeholder='$select_title' style='width:510px;' >";
	...
	protected function get_relation_input($field_info,$value)
	{
		...
		$ajax_or_not_class = $using_ajax ? 'selectpicker' : 'selectpicker';

		$this->_inline_js("var ajax_relation_url = '".$this->getAjaxRelationUrl()."';\n");

		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		$input = "<select id='field-{$field_info->name}' data-live-search='true' name='{$field_info->name}' class='$ajax_or_not_class' data-placeholder='$select_title' style='width:300px'>";
	...
	protected function get_relation_n_n_input($field_info_type, $selected_values)
	{
		...
		{
			$css_class = $has_priority_field || $is_ie_7 ? 'selectpicker': 'selectpicker';
			...
			$input = "<select id='field-{$field_info_type->name}' name='{$field_info_type->name}[]' multiple data-live-search='true' size='8' class='$css_class' data-placeholder='$select_title' style='$width_style' >";
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		$input= '';
		$input .='<div class="input-group form-line file-select">';
		$input .='<span class="input-group-btn">';
		$input .='<button class="ckfinder-popup btn btn-default" type="button"><i class="glyphicon glyphicon-upload"></i> Select file</button>';
		$input .='</span>';
		$input .= "<input id='field-{$field_info->name}' class='form-control' name='{$field_info->name}' type='text' value='$value'/>";
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	protected $default_assets_path					= 'grocery_crud'; // this is path after domain
	...
	{
		// joh fix
		$this->default_language_path				= FCPATH.'/grocery_crud/languages';
		$this->default_config_path					= FCPATH.'/grocery_crud/config';
	}
	...
	/**
	 *
	 * This fuction add extra fuction example you need add a function self make
	 * @param $label of your subject, for example "Photo Gallery".
	 * @param $url
	 * @param $css_class
	 * @param $url_callback
	 */
	public function add_extra_function( $label, $link_url = '', $css_class = '', $url_callback = null)
	{
		$unique_id = substr($label,0,1).substr(md5($label.$link_url),-8); //The unique id is used for class name so it must begin with a string

		$this->extra_functions[$unique_id]  = (object)array(
			'label' 		=> $label,
			'link_url'		=> $link_url,
			'css_class' 	=> $css_class,
			'url_callback' => $url_callback,
