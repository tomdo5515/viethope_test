...
class Event extends BackendController {
	...
	*/
	...
	public function index()
	{
		...
		$crud->display_as('article_title','Event title');
		$crud->display_as('article_status','Event status');
		$crud->display_as('article_author','Event author');
		$crud->display_as('article_date','Create date');

		if(!$this->ion_auth->is_admin())
		{
			$crud->unset_delete();
			$crud->columns('thumbnail','id','article_status','article_date');
		...
		$crud->where('articles.article_type', ArticleType::EVENT);
		
		$crud->add_extra_function('Add event', '/dashboard/event/addevent','glyphicon-plus');
		$crud->add_action('Edit', '', '/dashboard/event/editevent', 'glyphicon-pencil');
		// $crud->add_action('Delete', '', '/dashboard/event/deleteevent', 'glyphicon-trash');
	...
	public function addevent(){
		...
		$this->twig->set($data);
		$this->twig->display('event/event_add');
	}

	public function editevent($id){
		// get article
		$data['languages'] = $this->LanguageModel->get_languages();
		$data['article'] = $this->ArticleModel->get_event_default_lang($id);
		$data['article_id'] = $id;
		...
		$this->twig->set($data);
		$this->twig->display('event/event_edit');
	}
